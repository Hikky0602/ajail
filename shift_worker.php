...
<div id="schedule">
	...
	<div id="calender">
		...
		<tr>
		<!--初月の曜日揃え-->
<?php	for($j=0;$j<date_id($year,$month,1);$j++){	?>
			<td> 
		
<?php	}for($j=1;$j<=7-date_id($year,$month,1);$j++){	?>
			<td>
			<input type="button" class=“squareBt”  value=<?php echo $j ?>&nbsp&nbsp onClick=turn(<?php echo ($j-1) ?>)> 
<?php	}	?>
...
<!--
<?php/*		if($i<10){			?>
				<input type="button" name="sche[]" value=<?php echo $i ?>&nbsp&nbsp onClick=turn(<?php ($i-1) ?>) class=“squareBt” >" ;
<?php/*		}else{				*/?>
-->				<input type="button" name="sche[]" value=<?php echo $i ?> onClick=turn(<?php echo ($i-1) ?>) class=“squareBt” >
<?php	//	}
			if(($i-$j)%7==6){	?>
		</tr>
<?php	}}						?>
...
for($i=0;$i<$day;$i++){
	if(count($arr)>0){
		if($arr[$i]==1){	?>
			<input type="hidden" name="schedule_worker[]" value=1>
<?php	}else{				?>
			<input type="hidden" name="schedule_worker[]" value=0>
<?php	}
	}else{					?>
			<input type="hidden" name="schedule_worker[]" value=0>
<?php	}}	?>
